...
int getLastWinScore(Board boards[], int draws[]) {
    int winCnt = 0;
    bool hasWon[BOARDS_CNT] = {0};
    ...
    for (int drawInd = 0; drawInd < DRAWS_CNT; drawInd++) {
        ...
        for (int boardInd = 0; boardInd < BOARDS_CNT; boardInd++) {
            ...
            if (boards[boardInd].isWin()) {
                winCnt++;
                if (winCnt == BOARDS_CNT) {
                    return boards[boardInd].getScore(draw);
                }
                hasWon[boardInd] = true;
            }
        }
    }
}
...
int main()
{
    ...
    std::cout << getLastWinScore(boards, draws) << std::endl;
